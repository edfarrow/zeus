...
FancyUpload3.Attach = new Class({
	...
	onFileRemove: function() {
		if (this.fileList.length == 0) {
			this.select.setStyle('display', 'inline');
			this.reposition();
		}
	},
...
FancyUpload3.Attach.File = new Class({
	...
	render: function() {
		...
		var progress = new Element('img', { 'class': 'file-progress', src: '<%= WebResource("Zeus.Editors.Resources.FancyFileUpload.bar.gif") %>' }).inject(this.ui.size, 'after');
